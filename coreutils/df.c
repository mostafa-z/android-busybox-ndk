...
#ifdef __BIONIC__
/* Fix segmentation fault for getmntent() in android version */
struct mntent *getmntent__df_priv(FILE *stream);
struct mntent* FAST_FUNC find_mount_point__df_priv(const char *name, int subdir_too);
...
struct mntent* FAST_FUNC find_mount_point__df_priv(const char *name, int subdir_too)
{
	...
	while ((mountEntry = getmntent__df_priv(mtab_fp)) != NULL) {
...
int df_main(int argc UNUSED_PARAM, char **argv)
{
	...
	while (1) {
		...
		if (mount_table) {
#ifdef __BIONIC__
			mount_entry = getmntent__df_priv(mount_table);
#else
			...
			if (!mount_point)
				break;
#ifdef __BIONIC__
			mount_entry = find_mount_point__df_priv(mount_point, 1);
#else
